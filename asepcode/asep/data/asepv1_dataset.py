...
if __name__ == "__main__":
    ...
    data_dir = "/Users/mansoor/Documents/GSU/Projects/Antibody-Design/epitope-prediction/data/asep"
    asepv1_dataset = AsEPv1Dataset(
        root= data_dir,  #"/mnt/data", # "/asep/data",
        name="asep",
        # embedding setup
...
"""
The script asepv1_dataset.py is responsible for handling the dataset, including downloading, processing, and loading the data. 
Here's a breakdown of what each part of the script does:

Class Definitions:

PairData: Inherits from PygData and adds custom behavior for handling antibody and antigen graph pairs.
AsEPv1Dataset: Inherits from PygInMemoryDataset and handles the dataset-specific logic, including downloading, processing, and 
loading the data.
...
transform: A function/transform that takes in a torch_geometric.data.Data object and returns a transformed version.
pre_transform: A function/transform that takes in a torch_geometric.data.Data object and returns a transformed version before saving to disk.
pre_filter: A function that takes in a torch_geometric.data.Data object and returns a boolean value, indicating whether the data object should be included in the final dataset.
embedding_config: A dictionary containing the embedding configuration for the antibody and antigen graphs.

Behavior:
Sets the name and emb_config attributes.
Calls the parent class's __init__ method with the provided parameters.
Determines the node feature type (node_feat_type) from the embedding configuration and loads the appropriate data based on this type.
String Representation (__repr__ method):

Returns a string representation of the dataset, including its name and the number of data points.
Properties:

raw_dir: Returns the raw directory where the dataset files are stored.
processed_dir: Returns the processed directory where the processed data files are stored.
raw_file_names: Returns a list of raw file names used to check if files exist before downloading.
processed_file_names: Returns a list of processed file names used to save the processed data.
Download Method (download method):

Checks if the raw files already exist. If they do, it skips the download.
Downloads the dataset from the provided URL using the gdown library.
Extracts the downloaded files and moves them to the appropriate directories.
Process Method (process method):

Reads the list of antibody IDs from a file.
Reads the data into a list of PairData objects using the convert_one_interim_to_pyg_pair_data function.
Applies any pre-filter and pre-transform functions if provided.
Collates the data list into a single data object and saves it to the appropriate processed file based on the node feature type.
Split Index Methods (get_idx_split and get_idx_random_split methods):

get_idx_split: Returns the split indices for the dataset based on the provided split method (either 'epitope_ratio' or 'epitope_group').
get_idx_random_split: Returns a random split of the dataset.
Evaluator Class (AsEPv1Evaluator):

Initialization (__init__ method): Sets the bprc attribute to the binary_auprc function from the torcheval.metrics.functional module.
Evaluation Method (eval method): Evaluates the input dictionary containing y_pred and y_true tensors and returns the AUC-PRC metric.
Helper Functions:

sparse_matrix_to_edge_index: Converts a sparse matrix to an edge index tensor.
aa_seq_to_one_hot_tensor: Converts an amino acid sequence to a one-hot encoded tensor.
load_interim_graph_pkl: Loads interim graph data from a .pkl file.
load_interim_graph_pt: Loads interim graph data from a .pt file.
validate_custom_embedding_method: Validates a custom embedding method to ensure it has the correct signature and behavior.
get_node_feat_from_interim_graph_data: Gets node features from interim graph data, or computes node features using a custom method if provided.
convert_one_interim_to_pyg_pair_data: Converts one interim graph to a PyG PairData object.
Main Block:

Creates an EmbeddingConfig object with default settings.
Creates an AsEPv1Dataset object with the specified root directory, name, and embedding configuration.
Gets the split indices for the dataset using the get_idx_split method.
Splits the dataset into training, validation, and test sets based on the split indices.
