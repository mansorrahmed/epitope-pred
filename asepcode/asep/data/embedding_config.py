...
class CustomEmbeddingMethodSrc(BaseModel):
    ...
    def check_method_name(cls, v, values):
        script_path = values.data.get("script_path", None)
...
class ABConfig(BaseConfig):
    embedding_model: str = (
        "esm2"  # Default for 'ab', choices=['igfold', 'esm2', 'custom']
    )
    ...
    def adjust_embedding_model_by_src(cls, values):
        ...
        src = values.custom_embedding_method_src
    ...
    def check_custom_embedding_method_src(cls, values):
        ...
        custom_embedding_method = values.custom_embedding_method
        src = values.custom_embedding_method_src
...
class EmbeddingConfig(BaseModel):
    ...
    @field_validator("node_feat_type")
    ...
    def adjust_embedding_model(cls, values):
        node_feat_type = values.node_feat_type
